...
import { getLetter, deleteLetter, LetterData } from '@/app/utils/firebase';
import Image from 'next/image';
import Link from 'next/link';
import LetterPage from '@/app/components/LetterPage';

export default function MektupDetay() {
  const params = useParams();
  const router = useRouter();
  const [letter, setLetter] = useState<LetterData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const mektupId = params.id as string;

  useEffect(() => {
    async function fetchLetter() {
      try {
        setLoading(true);
        const letterData = await getLetter(mektupId);
        setLetter(letterData);
      } catch (err) {
        console.error('Mektup yüklenirken hata:', err);
        setError('Mektup yüklenemedi.');
      } finally {
        setLoading(false);
      }
    }
